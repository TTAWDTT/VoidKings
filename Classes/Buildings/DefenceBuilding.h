#include "cocos2d.h"
#include "DefenceBuildingData.h"

// 此处开始写防御建筑类
class DefenceBuilding : public cocos2d::Node {
public:
    // 标准 Cocos create 方法
	static DefenceBuilding* create(const DefenceBuildingConfig* config, int level = 0); // 创建建筑实例,默认等级为0
    virtual bool init(const DefenceBuildingConfig* config, int level = 0); // 初始化、绑定子节点
