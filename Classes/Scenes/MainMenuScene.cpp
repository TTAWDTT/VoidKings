...
void MainMenuScene::createMenuButtons() {
    ...
    auto createButton = [=](const std::string& text, const Color4F& bgColor,
        const Color4F& borderColor, const ccMenuCallback& callback) -> MenuItem* {
            ...
            // 绘制背景矩形
            bg->drawSolidRect(Vec2(0, 0),
                Vec2(buttonWidth, buttonHeight),
                bgColor);
            // 绘制边框 - 使用更粗的线条
            bg->drawRect(Vec2(0, 0),
                Vec2(buttonWidth, buttonHeight),
                borderColor);
            // 添加内部高亮线
            bg->drawLine(Vec2(2, buttonHeight - 2),
                Vec2(buttonWidth - 2, buttonHeight - 2),
                Color4F(1, 1, 1, 0.3f));
            //让背景中心对齐 Label 中心
            bg->setPosition(Vec2(
                button->getContentSize().width / 2 - buttonWidth / 2,
                button->getContentSize().height / 2 - buttonHeight / 2
            ));
            // 添加背景
...
void MainMenuScene::onHelp(Ref* sender) {
    ...
    closeBg->drawSolidRect(Vec2(0, 0), Vec2(100, 20), Color4F(0.5f, 0.3f, 0.3f, 1.0f));
    closeBg->drawRect(Vec2(0, 0), Vec2(100, 20), Color4F(0.7f, 0.4f, 0.4f, 1.0f));
    closeBg->setPosition(closeButton->getContentSize().width / 2 - 100 / 2, closeButton->getContentSize().height / 2 - 20 / 2);
    closeButton->addChild(closeBg, -1);
    closeButton->setPosition(Vec2(visibleSize.width / 2, panelY + 15));
