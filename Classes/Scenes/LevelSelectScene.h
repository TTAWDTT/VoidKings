...
namespace LevelSelectConfig {
    ...
    constexpr float LEVEL_BUTTON_SIZE = 80.0f;       // 关卡按钮尺寸
    constexpr float LEVEL_BUTTON_SPACING = 30.0f;    // 关卡按钮间距
    constexpr float LEVEL_AREA_Y_RATIO = 0.55f;      // 关卡区域Y位置（相对屏幕高度）
    
    // 兵种预览区域配置
    constexpr float UNIT_PREVIEW_HEIGHT = 120.0f;    // 兵种预览区域高度
    constexpr float UNIT_PREVIEW_BOTTOM = 20.0f;     // 距离底部距离
    constexpr float UNIT_ICON_SIZE = 60.0f;          // 兵种图标尺寸
    constexpr float UNIT_ICON_SPACING = 15.0f;       // 兵种图标间距
